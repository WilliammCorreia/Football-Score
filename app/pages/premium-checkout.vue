...
// Performance tracking : temps de chargement de la page de validation de commande
const loadStart = import.meta.client ? performance.now() : 0;

onMounted(() => {
  measureLoadTime('Checkout — validation commande', loadStart);
  track('checkout_start', { product: 'premium_subscription', price_cents: 499 });
});

// Simulation de panne : la passerelle de paiement échoue 1 fois sur 3
// avec un TypeError (accès à une propriété de null), remonté à GlitchTip
// avec sa stacktrace complète (OS, navigateur, ligne exacte).
function processPaymentGateway(amountCents: number) {
  if (Math.random() < 1 / 3) {
    const gateway = null as unknown as { process: (amountCents: number) => void };
    gateway.process(amountCents);
  }
}
...
async function handleConfirm() {
  ...
  try {
    ...
    if (error instanceof TypeError) {
      ...
      track('checkout_failed', { product: 'premium_subscription', reason: 'payment_gateway_error' });
      errorMessage.value = 'Le paiement a été refusé. Veuillez réessayer.';
    }
    else {
    ...
    <div class="w-full max-w-xl">
      ...
      <div class="card overflow-hidden">
        ...
        <div class="flex items-center justify-between gap-3 p-6">
          ...
          <span class="tabular font-mono text-xl font-semibold text-text-main">
            4,99 €
          </span>
        </div>
        ...
        <div class="flex items-center justify-between border-t border-border bg-surface-muted px-6 py-4">
          ...
          <span class="display tabular font-mono text-3xl text-primary-600">
            4,99 €
          </span>
        </div>
      </div>
      ...
      <div
        v-if="!successMessage"
        class="mt-6 flex flex-col-reverse gap-3 md:flex-row"
      >
        <NuxtLink
          to="/premium"
