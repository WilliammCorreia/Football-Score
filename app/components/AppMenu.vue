<script setup lang="ts">
const { user, clear: clearSession, loggedIn } = useUserSession();
async function logout() {
  await clearSession();
  await navigateTo('/login');
}
</script>

<template>
  <nav class="size-full bg-primary-200">
    <ul class="px-4 py-8 md:px-7 md:py-10">
      <li
        v-if="loggedIn"
        class="mb-2 flex flex-row items-center gap-2 rounded-xl p-3 text-xl font-semibold hover:bg-primary-300 active:bg-primary-400 md:mb-4"
      >
        <Icon
          name="ion:ios-home-outline"
          size="2rem"
          mode="svg"
        />
        <NuxtLink to="/dashboard">Dashboard</NuxtLink>
      </li>
      <li
        v-if="loggedIn"
        class="my-2 flex flex-row items-center gap-2 rounded-xl p-3 text-xl font-semibold hover:bg-primary-300 active:bg-primary-400"
      >
        <Icon
          name="ion:ios-football"
          size="2rem"
          mode="svg"
        />
        <NuxtLink to="/">Matchs</NuxtLink>
      </li>
      <li
        v-if="loggedIn"
        class="my-2 flex flex-row items-center gap-2 rounded-xl p-3 text-xl font-semibold hover:bg-primary-300 active:bg-primary-400"
      >
        <Icon
          name="ion:ios-search-strong"
          size="2rem"
          mode="svg"
        />
        <NuxtLink to="/search">Recherche</NuxtLink>
      </li>
      <li
        v-if="!user"
        class="my-2 flex flex-row items-center gap-2 rounded-xl p-3 text-xl font-semibold hover:bg-primary-300 active:bg-primary-400"
      >
        <Icon
          name="ion:ios-plus-outline"
          size="2rem"
          mode="svg"
        />
        <NuxtLink to="/register">Inscription</NuxtLink>
      </li>
      <li
        v-if="!user"
        class="my-2 flex flex-row items-center gap-2 rounded-xl p-3 text-xl font-semibold hover:bg-primary-300 active:bg-primary-400"
      >
        <Icon
          name="ion:ios-log-in"
          size="2rem"
          mode="svg"
        />
        <NuxtLink to="/login">Login</NuxtLink>
      </li>

      <li
        v-if="loggedIn"
        class="my-2 flex flex-row items-center gap-2 rounded-xl p-3 text-xl font-semibold hover:bg-primary-300 active:bg-primary-400"
      >
        <Icon
          name="ion:ios-settings"
          size="2rem"
          mode="svg"
        />
        <NuxtLink to="/settings">Paramètres</NuxtLink>
      </li>
      <li
        v-if="user"
        class="my-2 flex flex-row items-center gap-2 rounded-xl p-3 text-xl font-semibold hover:bg-primary-300 hover:text-danger active:bg-primary-400"
      >
        <Icon
          name="ion:ios-log-out"
          size="2rem"
          mode="svg"
        />
        <button @click="logout">
          Déconnexion
        </button>
      </li>
    </ul>
  </nav>
</template>
